...
record_log = True
record_log_dir = './log/square_loss/'
...
with tf.variable_scope('layer1'):
    ...
    layer1.batch_norm(layer1.conv2d([5, 5], 64, [1, 2, 2, 1]), is_training=is_training)
    layer1.set_output(tf.nn.sigmoid(layer1.bn))
    
# ===================24-->12
with tf.variable_scope('layer2'):
    layer2 = ConvObj()
    layer2.set_input(layer1.output)
    layer2.batch_norm(layer2.conv2d([5, 5], 128, [1, 2, 2, 1]), is_training=is_training)
    layer2.set_output(tf.nn.sigmoid(layer2.bn))
    
# ===================12-->6
with tf.variable_scope('layer3'):
    layer3 = ConvObj()
    layer3.set_input(layer2.output)
    layer3.batch_norm(layer3.conv2d([5, 5], 256, [1, 2, 2, 1]), is_training=is_training)
    layer3.set_output(tf.nn.sigmoid(layer3.bn))
    
# ===================6-->3
with tf.variable_scope('layer4'):
    layer4 = ConvObj()
    layer4.set_input(layer3.output)
    layer4.batch_norm(layer4.conv2d([5, 5], 512, [1, 2, 2, 1]), is_training=is_training)
    layer4.set_output(tf.nn.sigmoid(layer4.bn))

# ===================3*3*512-->pts
with tf.variable_scope('layer5'):
    layer5 = FcObj()
    layer5.set_input(layer4.output)
    layer5.batch_norm(layer5.fc(layer5.input_size), is_training=is_training)
    layer5.set_output(tf.nn.sigmoid(layer5.bn))
    
with tf.variable_scope('layer6'):
    layer6 = FcObj()
    layer6.set_input(layer5.output)
    layer6.fc(pts_train.shape.as_list()[1])
    layer6.set_output(layer6.logit)
    


with tf.variable_scope('loss'):
    weight_loss = tf.reduce_sum(tf.get_collection(tf.GraphKeys.REGULARIZATION_LOSSES))
    cross_entropy = tf.reduce_mean(tf.square(layer6.output - ph_pts))
    total_loss = cross_entropy + weight_loss
    accuarcy = tf.reduce_mean(tf.abs(layer6.output - ph_pts))
...
tf.train.write_graph(sess.graph_def, record_log_dir, "graph.pb", as_text=True);
